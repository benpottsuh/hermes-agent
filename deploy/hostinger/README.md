...
| Where | How |
|--------|-----|
| **Terminal** | Cursor → **View → Terminal** |
| **Directory** | `cd ~/projects/hermes-agent` |
| **Run** | `./deploy/hostinger/bootstrap.sh` |

See **[BOOTSTRAP.md](./BOOTSTRAP.md)** for what is automated vs what you do manually (with hPanel paths).

## Auto-update server info for Cursor

After SSH works, copy and edit the env file once:

```bash
cp deploy/hostinger/hostinger.env.example deploy/hostinger/hostinger.env
# SSH_HOST=hermes-hostinger, HERMES_INSTALL=/opt/hermes, HERMES_USER=hermes
```

Refresh facts anytime (no secrets in the output file):

```bash
./deploy/hostinger/collect-context.sh
```

Cursor reads `deploy/hostinger/.server-context.md` automatically. Re-run after deploys or when the server changes.

Optional: add to your shell after SSH succeeds, or chain after sync:

```bash
./deploy/hostinger/sync.sh hermes-hostinger /opt/hermes && ./deploy/hostinger/collect-context.sh
```
